...
export interface ThreadParams {
  ...
  /** Core-owned tone property, mapped to Codex's `personality` preset; absent = 'friendly' (the pre-property hardcode). */
  tone?: string;
}
...
export async function startOrResumeCodexThread(
  ...
  const commonParams = {
    ...
    personality: params.tone ?? 'friendly',
...
// Per-capability plan sections. The runtime contract binds each of the four
// configuration responsibilities to one of these; the same functions build
// the legacy writer's plan, so both write paths share every byte.
