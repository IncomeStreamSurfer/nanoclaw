...
export async function composeGroupProjectDoc(group: AgentGroup, groupDir: string, spec: ProjectDocSpec): Promise<void> {
  ...
  const baseDoc = path.resolve(process.cwd(), BASE_INSTRUCTIONS_PATH);
  if (fs.existsSync(baseDoc)) {
    push(BASE_DOC_SECTION, renderBaseInstructions(fs.readFileSync(baseDoc, 'utf-8'), spec.instructions));
  } else {
