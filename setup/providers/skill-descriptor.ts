...
export function listProviderDescriptors(projectRoot = process.cwd()): InstallableProviderDescriptor[] {
  ...
  for (const entry of entries) {
    ...
    const descriptor = parseDescriptor(fs.readFileSync(skillFile, 'utf-8'), entry);
...
function parseDescriptor(
  ...
  const installSkill = required(metadata, 'install-skill', directory);
  ...
  if (installSkill !== directory) throw new Error(`${directory}: install-skill must match its directory`);
  if (image !== 'local-required' && image !== 'hardened-compatible') {
    throw new Error(`${directory}: invalid provider image policy '${image}'`);
  }
  return { value, label, hint, installSkill, image, offered: offered === 'true' };
}
