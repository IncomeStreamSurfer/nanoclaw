...
const COMBINED_PROVIDERS = ['add-codex', 'add-opencode'];
...
async function testCombinedProviders(skills: RegistrySkill[]): Promise<void> {
  const selected = COMBINED_PROVIDERS.map((skill) => skills.find((candidate) => candidate.skill === skill));
  if (selected.some((skill) => !skill)) throw new Error('combined provider skills are missing');
  ...
  try {
    ...
    for (const meta of selected as RegistrySkill[]) {
      console.log(`\n==> ${meta.skill}`);
...
async function testOldProviderRefresh(commit: string): Promise<void> {
  ...
  try {
    ...
    const oldSkills = COMBINED_PROVIDERS.map((name) =>
...
async function testPreContractProviders(): Promise<void> {
  ...
  try {
    ...
    for (const name of COMBINED_PROVIDERS) {
